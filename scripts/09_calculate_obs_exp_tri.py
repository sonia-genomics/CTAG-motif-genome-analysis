...
genome_folder = "data/processed/genomes_singleline"
genes_folder = "data/processed/genes"
termination_file = "output/last4_nt.xlsx"

output_path = "output/final_tri_analysis.xlsx"

# -------- TRI --------
bases = ["A","T","G","C"]
trinucleotides = ["".join(p) for p in itertools.product(bases, repeat=3)]
...
with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
    ...
    for genome_file in os.listdir(genome_folder):
        ...
        if genome_file.endswith(".fna"):
            ...
            kmer_counts_genome = Counter(genome_seq[i:i+3] for i in range(len(genome_seq)-2))
            ...
            for tri in trinucleotides:
                f1 = {"A":fa,"T":ft,"G":fg,"C":fc}[tri[0]]
                f2 = {"A":fa,"T":ft,"G":fg,"C":fc}[tri[1]]
                f3 = {"A":fa,"T":ft,"G":fg,"C":fc}[tri[2]]
                genome_expected_dict[tri] = f1 * f2 * f3 * genome_len
            ...
            kmer_counts_coding = Counter(coding_seq[i:i+3] for i in range(len(coding_seq)-2))
            ...
            if matched_sheet:
                ...
                if "Last_3_nt" in term_df.columns:
                    last3_list = term_df["Last_3_nt"].dropna().str.upper().tolist()
                else:
                    print(f"Last_3_nt column missing in {matched_sheet}")
                    last3_list = []

                term_counts = Counter(last3_list)
                term_len = len(last3_list) * 3
